...
const CreateAssignmentDialog = ({ open, setOpen }: Props) => {
	const [title, setTitle] = useState<string>("");
	const [platform, setPlatform] = useState<string>("");
	const [deadline, setDeadline] = useState<string>(new Date().toDateString());
	...
	const handleSubmit = () => {
		...
		addAssignment({
			title,
			platform: platform as "GOOGLECLASSROOM" | "LMS" | "TEAMS",
			deadline,
		});
		handleClose();
	};
	...
	return (
		...
		>
			...
			>
				<Typography variant="h5">Add a new assignment</Typography>
				<Box mt={2}>
					<TextField
						label="Title"
						variant="outlined"
